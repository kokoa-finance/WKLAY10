...
import "./interfaces/IWKLAY10.sol";
...
contract WKLAY10 is IWKLAY10 {

    string public constant name = "Wrapped Klay v10";
    string public constant symbol = "WKLAY10";
    ...
    function flashFee(address token, uint256) external view override returns (uint256) {
        require(token == address(this), "WKLAY: flash mint only WKLAY10");
        return 0;
    }
    ...
    function flashLoan(IERC3156FlashBorrower receiver, address token, uint256 value, bytes calldata data) external override returns (bool) {
        require(token == address(this), "WKLAY: flash mint only WKLAY10");
        require(value <= type(uint112).max, "WKLAY: individual loan limit exceeded");
        flashMinted = flashMinted + value;
        require(flashMinted <= type(uint112).max, "WKLAY: total loan limit exceeded");
        ...
        require(balance >= value, "WKLAY: burn amount exceeds balance");
    ...
    function withdrawFrom(address from, address payable to, uint256 value) external override {
        if (from != msg.sender) {
            ...
            if (allowed != type(uint256).max) {
                require(allowed >= value, "KLAY: request exceeds allowance");
                uint256 reduced = allowed - value;
    ...
    function permit(address owner, address spender, uint256 value, uint256 deadline, uint8 v, bytes32 r, bytes32 s) external override {
        require(block.timestamp <= deadline, "WKLAY: Expired permit");
        ...
        require(signer != address(0) && signer == owner, "WKLAY: invalid permit");
    ...
    function transfer(address to, uint256 value) external override returns (bool) {
        ...
        if (to != address(0)) { // Transfer
            uint256 balance = balanceOf[msg.sender];
            require(balance >= value, "WKLAY: transfer amount exceeds balance");
